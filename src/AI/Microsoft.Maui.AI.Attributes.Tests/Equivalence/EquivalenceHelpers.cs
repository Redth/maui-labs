...
internal static class EquivalenceHelpers
{
    ...
    {
        var e = Normalize(expected, options);
        var a = Normalize(actual, options);
        var en = JsonSerializer.SerializeToNode(e, AIJsonUtilities.DefaultOptions);
        var an = JsonSerializer.SerializeToNode(a, AIJsonUtilities.DefaultOptions);
